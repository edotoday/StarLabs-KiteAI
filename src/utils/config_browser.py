...
def load_config():
    ...
    try:
        ...
        with open(config_path, "r") as file:
            ...
            # Ensure RPCS has KITEAI field
            if "RPCS" in config:
                rpcs_defaults = {"KITEAI": ["https://rpc.kitechain.org"]}
            ...
            # Ensure GOKITE section exists in STAKINGS
            if "GOKITE" not in config["STAKINGS"]:
                config["STAKINGS"]["GOKITE"] = {}

            gokite_defaults = {
                "KITE_AMOUNT_TO_STAKE": [1.0, 2.0],
                "UNSTAKE": False,
            }

            for key, default_value in gokite_defaults.items():
                if key not in config["STAKINGS"]["GOKITE"]:
                    config["STAKINGS"]["GOKITE"][key] = default_value
...
def save_config(config):
    ...
    try:
        ...
        # Убедимся, что раздел GOKITE содержит все необходимые поля
        if "GOKITE" not in config["STAKINGS"]:
            config["STAKINGS"]["GOKITE"] = {}
...
<head>
    ...
    <title>StarLabs KiteAI Configuration</title>
    ...
    <div class="app-container">
        <header>
            <div class="logo">
                ...
                <h1>StarLabs KiteAI Configuration</h1>
            </div>
    ...
    Object.entries(sections).forEach(([sectionId, { key, title, icon }], index) => {
        ...
        if (config[key]) {
            if (key === 'SETTINGS') {
                ...
                // GOKITE
                if (config[key]['GOKITE']) {
                    createCard(cardsContainer, 'GoKite Staking Settings', 'coins', 
                        Object.entries(config[key]['GOKITE']).map(([k, v]) => ({ 
                            key: k, 
                            value: v, 
