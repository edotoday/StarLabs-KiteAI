...
class Solvium:
    ...
    async def create_recaptcha_task(
        ...
        """Creates a reCAPTCHA solving task for v2 or v3"""
        headers = {
            "Authorization": f"Bearer {self.api_key}",
        }

        # Choose endpoint based on version
        endpoint = "recaptcha-v2" if version == "v2" else "recaptcha-v3"
        url = f"{self.base_url}/task/{endpoint}?url={pageurl}&sitekey={sitekey}&ref=starlabs"
        ...
        try:
            ...
            logger.error(f"Error creating reCAPTCHA {version} task with Solvium: {result}")
            return None

        except Exception as e:
            logger.error(f"Error creating reCAPTCHA {version} task with Solvium: {e}")
            return None
    ...
    async def solve_recaptcha(
        ...
        """Solves reCAPTCHA v2 or v3 and returns token"""
